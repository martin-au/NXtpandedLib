...
static __STK_UNIT _stack_Task2[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 4, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 1, TPRI_MINTASK + 4, };
const UINT8 tinib_maxact[TNUM_TASK] = { (1) - 1, (1) - 1, };
const AppModeType tinib_autoact[TNUM_TASK] = { 0x00000000, 0x00000000, };
const FP tinib_task[TNUM_TASK] = { TASKNAME( TaskMain ), TASKNAME( Task2 ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_TaskMain, (__STK_UNIT)_stack_Task2, };
...
const AlarmType cyclic_alarm_main = 0;
const AlarmType cyclic_alarm_task2 = 1;

DeclareTask(TaskMain);
static void _activate_alarm_cyclic_alarm_main( void );
static void _activate_alarm_cyclic_alarm_main( void )
{ (void)ActivateTask( TaskMain ); }

DeclareTask(Task2);
static void _activate_alarm_cyclic_alarm_task2( void );
static void _activate_alarm_cyclic_alarm_task2( void )
{ (void)ActivateTask( Task2 ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_cyclic_alarm_main, _activate_alarm_cyclic_alarm_task2, };
const AppModeType alminib_autosta[TNUM_ALARM] = { 0x00000001, 0x00000001, };
const TickType alminib_almval[TNUM_ALARM] = { 1, 1, };
const TickType alminib_cycle[TNUM_ALARM] = { 10000, 3000, };
...
const ResourceType ostreamRes = 0;
const ResourceType motorARes = 1;
...
const EventMaskType EventSleep = (1UL << 0);
