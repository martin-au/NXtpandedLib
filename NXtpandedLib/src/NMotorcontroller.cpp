...
#include "NMotorcontroller.hpp"

#include "..\..\..\GNUARM\include\c++\4.0.2\cstdlib" // abs

void NMotorcontroller::controllerOff() {
...
void NMotorcontroller::moveRel(S32 pos, S16 pwr, bool waitMove, S32 off) {
   pos = -pos;
   controlMtx.acquire();
   if(pwr > 0)
      phigh = pwr;
...
void NMotorcontroller::moveAbs(S32 pos, S16 pwr, bool waitMove) {
   pos = -pos;
   if(pwr > 0) {
	  controlMtx.acquire();
      phigh = pwr;
...
S32 NMotorcontroller::getAbsPos() const {
...
void NMotorcontroller::resetMotorPos(S16 pwr, bool waitReset) {
