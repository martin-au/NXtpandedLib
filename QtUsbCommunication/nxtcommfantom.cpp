...
int NXTCommFantom_getName(nFANTOM100_iNXT nxt, ViChar* name)
{
    ...
    if(NXTCommFantom_send(nxt, (unsigned char*)buf, 0, 2) > 0)
	{
		while(timeout-- > 0)
		{
            read_len = NXTCommFantom_receive(nxt, (unsigned char*)buf, 0, 33);
			if (read_len > 0) break;
		}
 ...
 */
int NXTCommFantom_send(nFANTOM100_iNXT nxt, const unsigned char* data, int offset, int len)
{
	ViStatus status = 0;
 ...
 */
int NXTCommFantom_receive(nFANTOM100_iNXT nxt, unsigned char* data, int offset, int len)
{
	ViStatus status = 0;
